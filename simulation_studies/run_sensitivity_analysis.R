################################################################################
##  Zero-Inflation Sensitivity Analysis: Master Orchestrator (Normalized Scales)
##
##  This script orchestrates the sensitivity analysis by running each
##  scenario (1 to 15) in a separate, isolated R process.
##
##  The RMSE and Bias metrics are normalized using BCF-Linear as the baseline
##  (value = 1.0) for each zero-inflation level, enabling clear relative
...
cat("=== Starting Zero-Inflation Sensitivity Master Orchestrator ===\n")
...
for (i in 1:15) {
  checkpoint_file <- file.path(RESULTS_DIR, sprintf("scenario_results_%d.csv", i))
  ...
  if (file.exists(checkpoint_file)) {
    ...
    cmd <- sprintf("Rscript simulation_studies/run_single_scenario.R %d", i)
    exit_code <- system(cmd)
    
    if (exit_code != 0) {
      stop(sprintf("\nError: Scenario %d failed with exit code %d. Execution stopped.\n", i, exit_code))
    }
  }
}
...
write.csv(all_results, file.path(RESULTS_DIR, "sensitivity_analysis_results.csv"), row.names = FALSE)
cat(sprintf("[SUCCESS] Master CSV saved to: %s\n", file.path(RESULTS_DIR, "sensitivity_analysis_results.csv")))
...
# Normalize RMSE and Bias by BCF-Linear (basis = 1.0) at each DGP and c_shift level
norm_metrics <- long_results %>%
  filter(Metric %in% c("RMSE", "Bias")) %>%
  group_by(DGP, c_shift, Metric) %>%
  mutate(
    basis_val = value[Model == "BCF-Linear"],
    value_norm = value / basis_val
  ) %>%
  ungroup() %>%
  select(DGP, c_shift, Zero_Proportion, Model, Metric, value = value_norm) %>%
...
raw_metrics <- long_results %>%
  ...
  select(DGP, c_shift, Zero_Proportion, Model, Metric, value)
...
plot_dgp_sensitivity <- function(dgp_name, file_name) {
  ...
  p <- ggplot(dgp_data, aes(x = Zero_Proportion, y = value, color = Model, group = Model)) +
    ...
    labs(
      title = paste("Sensitivity Analysis (Normalized):", dgp_name),
      subtitle = "RMSE and Bias are normalized using BCF-Linear as the baseline (1.0) at each level",
...
# Generate plots
plot_dgp_sensitivity("DGP A: Log-Normal Hurdle", "dgp_a_sensitivity.png")
plot_dgp_sensitivity("DGP B: Gaussian Hurdle", "dgp_b_sensitivity.png")
plot_dgp_sensitivity("DGP C: Tweedie Compound", "dgp_c_sensitivity.png")

cat("\n[SUCCESS] Sensitivity analysis complete. Plots generated successfully!\n")
