...
// [[Rcpp::export]]
List pathe_bcfCore(arma::vec y_,
                   arma::vec I_,
                   ...
                   int status_interval = 100)
{
  ...
  // Activeoutcome fits maintained separately
  ...
  // MCMC Loop                                                               //
  //-------------------------------------------------------------------------//
  Rcout << "\nBeginning Path E Joint Copula (Active Outcome) BCF MCMC:\n";
  time_t tp;
  int time1 = time(&tp);
  size_t save_ctr = 0;
  ...
  for(size_t i = 0; i < total_iterations; i++) {
    ...
    // --- Step 3: Update Selection forests (t_sel_con, t_sel_mod) on full sample ---
    ...
    // --- Step 4: Update Outcome forests (t_out_con, t_out_mod) on ACTIVE sample only ---
    ...
    // --- Step 5: Fast forward prediction pass to get outcome fits for all n units ---
    for(size_t k=0; k<n; k++) {
    ...
    // Synchronize active fits vectors
    for(size_t idx=0; idx<n_c; idx++) {
      size_t k = active_indices[idx];
    ...
    // Outcome Prognostic scale (updated on active sample)
    for(size_t idx=0; idx<n_c; idx++) {
      size_t k = active_indices[idx];
      double target_out = V[k] - beta * (W[k] - (allfit_sel_con[k] + allfit_sel_mod[k]));
    ...
    // Outcome Moderating scale (updated on active sample)
    for(size_t idx=0; idx<n_c; idx++) {
      size_t k = active_indices[idx];
      double target_out = V[k] - beta * (W[k] - (allfit_sel_con[k] + allfit_sel_mod[k]));
