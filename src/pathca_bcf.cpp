...
// Path C-A: Collapsed Joint Copula-BCF with Active-Subset Outcome Intensity
// AND Active-Only Covariance Update.
//
// Path C-A extends Path E with two corrections inspired by Path A's variance
// control:
//   (i)  beta / sigma0^2 conjugate update is restricted to the active subset
//        (the augmented V's for inactive units only reinforce the current beta,
//        amplifying Tweedie misspecification noise; restricting tightens beta).
//   (ii) Path A's data-adaptive prior calibration (sd_control, sd_moderate,
//        lambda) is applied externally in the R wrapper before this Core call.
//
// All other MCMC mechanics are identical to pathe_bcfCore.

// [[Rcpp::export]]
List pathca_bcfCore(arma::vec y_,
                    arma::vec I_,
                    arma::mat Omega_sel_con, arma::mat Omega_sel_mod,
                    arma::mat Omega_out_con, arma::mat Omega_out_mod,
                    arma::mat Omega_out_con_full, arma::mat Omega_out_mod_full,
                    ...
                    NumericVector x_out_con_full_, NumericVector x_out_mod_full_,
                    ...
                    int status_interval = 100)
{
  ...
  for(NumericVector::iterator it = I_.begin(); it != I_.end(); ++it) {
    I.push_back(*it);
    if(*it == 1.0) {
      active_indices.push_back(idx_counter);
    }
    idx_counter++;
  }
  size_t n_c = active_indices.size();
  ...
  size_t p_out_mod = x_out_mod.size()/n_c;
  ...
  std::vector<double> x_out_con_full;
  for(NumericVector::iterator it = x_out_con_full_.begin(); it != x_out_con_full_.end(); ++it) {
    x_out_con_full.push_back(*it);
  }

  std::vector<double> x_out_mod_full;
  for(NumericVector::iterator it = x_out_mod_full_.begin(); it != x_out_mod_full_.end(); ++it) {
    x_out_mod_full.push_back(*it);
  }
  ...
  double* allfit_out_con_active = new double[n_c];
  double* allfit_out_mod_active = new double[n_c];
  ...
  for(size_t i=0; i<n_c; i++) {
    allfit_out_con_active[i] = 0.0;
    allfit_out_mod_active[i] = 0.0;
  }

  double* r_sel_con = new double[n];
  double* r_sel_mod = new double[n];
  double* r_out_con = new double[n_c];
  double* r_out_mod = new double[n_c];
  ...
  dinfo di_out_con;
  di_out_con.n = n_c; di_out_con.p = p_out_con; di_out_con.x = &x_out_con[0]; di_out_con.y = r_out_con;
  di_out_con.basis_dim = Omega_out_con.n_rows; di_out_con.omega = &Omega_out_con[0];

  dinfo di_out_mod;
  di_out_mod.n = n_c; di_out_mod.p = p_out_mod; di_out_mod.x = &x_out_mod[0]; di_out_mod.y = r_out_mod;
  di_out_mod.basis_dim = Omega_out_mod.n_rows; di_out_mod.omega = &Omega_out_mod[0];

  dinfo di_out_con_full;
  di_out_con_full.n = n; di_out_con_full.p = p_out_con; di_out_con_full.x = &x_out_con_full[0];
  di_out_con_full.basis_dim = Omega_out_con_full.n_rows; di_out_con_full.omega = &Omega_out_con_full[0];

  dinfo di_out_mod_full;
  di_out_mod_full.n = n; di_out_mod_full.p = p_out_mod; di_out_mod_full.x = &x_out_mod_full[0];
  di_out_mod_full.basis_dim = Omega_out_mod_full.n_rows; di_out_mod_full.omega = &Omega_out_mod_full[0];

  double* ftemp = new double[n];
  double* ftemp_active = new double[n_c];
  ...
  for(size_t j=0; j<ntree_out_con; ++j) {
    node_pointers_out_con[j].resize(n_c);
    fit_basis(t_out_con[j], xi_out_con, di_out_con, ftemp_active, node_pointers_out_con[j], true, true);
  }
  for(size_t j=0; j<ntree_out_mod; ++j) {
    node_pointers_out_mod[j].resize(n_c);
    fit_basis(t_out_mod[j], xi_out_mod, di_out_mod, ftemp_active, node_pointers_out_mod[j], true, false);
  }
  ...
  // MCMC Loop                                                               //
  //-------------------------------------------------------------------------//
  Rcout << "\nBeginning Path C-A Joint Copula (Active Outcome + Active Covariance) BCF MCMC:\n";
  time_t tp;
  int time1 = time(&tp);
  size_t save_ctr = 0;
  ...
  for(size_t i = 0; i < total_iterations; i++) {
    ...
    //   These augmented V's are used ONLY to update selection forests' targets
    //   (which depend on V via beta/sigma2 * (V - eta_c)). They are NOT used
    //   in the conjugate (beta, sigma0^2) update — see Step 7.
    for(size_t k=0; k<n; k++) {
      if(I[k] != 1.0) {
        double eta_b_k = allfit_sel_con[k] + allfit_sel_mod[k];
        double eta_c_k = allfit_out_con[k] + allfit_out_mod[k];
    ...
    // --- Step 3: Update Selection forests (full sample) ---
    ...
    // --- Step 4: Update Outcome forests on ACTIVE sample only ---
    pi_out_con.sigma = sigma0 / fabs(pi_out_con.eta);
    pi_out_mod.sigma = sigma0 / fabs(pi_out_mod.eta);

    for(size_t j=0; j<ntree_out_con; j++) {
      fit_basis(t_out_con[j], xi_out_con, di_out_con, ftemp_active, node_pointers_out_con[j], false, true);

      for(size_t idx=0; idx<n_c; idx++) {
        size_t k = active_indices[idx];
        double target_out = V[k] - beta * (W[k] - (allfit_sel_con[k] + allfit_sel_mod[k]));
        allfit_out_con_active[idx] = allfit_out_con_active[idx] - pi_out_con.eta * ftemp_active[idx];
        r_out_con[idx] = (target_out - (allfit_out_con_active[idx] + allfit_out_mod_active[idx])) / pi_out_con.eta;
      }
      ...
      for(size_t idx=0; idx<n_c; idx++) {
        allfit_out_con_active[idx] += pi_out_con.eta * ftemp_active[idx];
      }
    }

    for(size_t j=0; j<ntree_out_mod; j++) {
      fit_basis(t_out_mod[j], xi_out_mod, di_out_mod, ftemp_active, node_pointers_out_mod[j], false, false);

      for(size_t idx=0; idx<n_c; idx++) {
        size_t k = active_indices[idx];
        double target_out = V[k] - beta * (W[k] - (allfit_sel_con[k] + allfit_sel_mod[k]));
        allfit_out_mod_active[idx] = allfit_out_mod_active[idx] - pi_out_mod.eta * ftemp_active[idx];
        r_out_mod[idx] = (target_out - (allfit_out_con_active[idx] + allfit_out_mod_active[idx])) / pi_out_mod.eta;
      }
      ...
      for(size_t idx=0; idx<n_c; idx++) {
        allfit_out_mod_active[idx] += pi_out_mod.eta * ftemp_active[idx];
      }
    }

    // --- Step 5: Fast forward prediction pass to get outcome fits for all n ---
    for(size_t k=0; k<n; k++) {
      allfit_out_con[k] = pi_out_con.eta * fit_i_basis(k, t_out_con, xi_out_con, di_out_con_full, true);
      allfit_out_mod[k] = pi_out_mod.eta * fit_i_basis(k, t_out_mod, xi_out_mod, di_out_mod_full, false);
    }
    for(size_t idx=0; idx<n_c; idx++) {
      size_t k = active_indices[idx];
      allfit_out_con_active[idx] = allfit_out_con[k];
      allfit_out_mod_active[idx] = allfit_out_mod[k];
    }

    // --- Step 6: Update PX Scaling Factors (eta) ---
    double eta_old;
    ...
    for(size_t idx=0; idx<n_c; idx++) {
      size_t k = active_indices[idx];
      double target_out = V[k] - beta * (W[k] - (allfit_sel_con[k] + allfit_sel_mod[k]));
      ftemp_active[idx] = target_out - allfit_out_mod_active[idx];
    }
    eta_old = pi_out_con.eta;
    update_scale(ftemp_active, allfit_out_con_active, n_c, sigma0, pi_out_con, gen);
    for(size_t k=0; k<n; ++k) {
      allfit_out_con[k] = allfit_out_con[k] * pi_out_con.eta / eta_old;
    }
    for(size_t idx=0; idx<n_c; ++idx) {
      allfit_out_con_active[idx] = allfit_out_con_active[idx] * pi_out_con.eta / eta_old;
    }
    pi_out_con.sigma = sigma0 / fabs(pi_out_con.eta);
    ...
    for(size_t idx=0; idx<n_c; idx++) {
      size_t k = active_indices[idx];
      double target_out = V[k] - beta * (W[k] - (allfit_sel_con[k] + allfit_sel_mod[k]));
      ftemp_active[idx] = target_out - allfit_out_con_active[idx];
    }
    eta_old = pi_out_mod.eta;
    update_scale(ftemp_active, allfit_out_mod_active, n_c, sigma0, pi_out_mod, gen);
    for(size_t k=0; k<n; ++k) {
      allfit_out_mod[k] = allfit_out_mod[k] * pi_out_mod.eta / eta_old;
    }
    for(size_t idx=0; idx<n_c; ++idx) {
      allfit_out_mod_active[idx] = allfit_out_mod_active[idx] * pi_out_mod.eta / eta_old;
    }
    pi_out_mod.sigma = sigma0 / fabs(pi_out_mod.eta);

    // --- Step 7: Joint Conjugate Gibbs Update for (beta, sigma0^2) ---
    //   ACTIVE-ONLY: we sum residuals only over units with informative V
    //   (where V_k = scaled log Y_k is observed). The augmented V_k for
    //   inactive units carry no independent information about beta (they
    //   were drawn from a Normal centred at beta * delta_k), so including
    //   them reinforces the current beta and propagates DGP misspecification
    //   noise into the joint covariance.
    double S_xx = 0.0;
    double S_xy = 0.0;
    double S_yy = 0.0;
    ...
    if((i >= burn) && ((i - burn) % thin == 0)) {
      ...
      for(size_t k=0; k<n; k++) {
        ...
        out_tau_post(save_ctr, k) = pi_out_mod.eta * fit_i_basis(k, t_out_mod, xi_out_mod, di_out_mod_full, true);
  ...
  delete[] allfit_out_con_active;
  delete[] allfit_out_mod_active;
  delete[] r_sel_con;
  delete[] r_sel_mod;
  delete[] r_out_con;
  delete[] r_out_mod;
